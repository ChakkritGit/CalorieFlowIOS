...
## สถานะ: โมเดลทำงานบนเครื่องจริงแล้ว

ยืนยันบน iPhone 17 Pro (iOS 26, Apple Intelligence ถูก restricted จึงตกมาที่ Core ML จริง ๆ)
โหลดโมเดลใน 1.7-6.6 วินาที การ์ดโค้ชได้ข้อความที่โมเดลเขียนเองและอ้างตัวเลขจากข้อมูลจริง

**`computeUnits` ต้องเป็น `.cpuAndGPU` เท่านั้น** — ANE สร้าง execution plan ให้กราฟนี้
ไม่ได้ ล้มตั้งแต่ `MLModel.load` วัดด้วยไฟล์ตัวจริงบนเครื่อง:

| computeUnits | ผล |
| --- | --- |
| `.cpuAndNeuralEngine` | error -14 |
| `.all` | error -14 (Core ML ไม่ถอยไปทางอื่นให้) |
| `.cpuAndGPU` | โหลดผ่าน 1.7-6.6 วินาที |

> เดิมโค้ดตั้ง `.cpuAndNeuralEngine` เพื่อเลี่ยง MPSGraph ที่เคย assert ตาย ซึ่งเป็น
> อาการของกราฟรุ่นเก่าที่มีมิติเปลี่ยนได้ รุ่นปัจจุบันไม่มีแล้ว เหตุผลนั้นหมดอายุ
...
1. **วัดความเร็วจริงบนเครื่อง** — ยังไม่ได้จับ token/วินาที กับแรมตอน inference
   บนมือถือ (บนแมคได้ 8-23 tok/s)
2. **คุณภาพคำตอบระดับ 1.5B** — แชทที่มีประวัติต้องยุบเป็นบรรทัดเดียว ไม่งั้นโมเดล
   ปิดเทิร์นทันที (ดู `AICoach.chatHistoryBlock`) และคำแนะนำยังหลุดประเด็นได้
   ถ้าจะดันต่อ: ลด temperature หรือขยับไป Qwen2.5-3B
3. **ฟอนต์ Anuphan** — ถ้าอยากให้ตรงกับเว็บเป๊ะ ต้องเพิ่ม `INFOPLIST_KEY_UIAppFonts`

## แปดบั๊กที่ต้องแก้กว่าจะได้โมเดลที่ใช้งานได้
...
| โหลดโมเดลไม่ผ่านบนเครื่องจริง (-14) | ANE สร้าง execution plan ให้กราฟนี้ไม่ได้ ต้องใช้ `.cpuAndGPU` | `CoreMLBackend.swift` |
| อาการสลับไปมาทุกครั้งที่เปิดแอป | ธง in-flight ค้างเมื่อโหลด *โยน error* (ไม่ได้แครช) รอบถัดไปเลยปิดโมเดลทิ้ง | `CoreMLBackend.swift` |
